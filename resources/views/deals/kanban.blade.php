    ...
    <div class="py-8" x-data="kanban()">
        ...
        <div class="flex flex-col md:flex-row gap-6 overflow-x-auto pb-8 snap-x">
            ...
            <div style="background: var(--nav-bg); border-radius: 2rem; border: 1px solid var(--nav-border); min-width: 320px; flex: 1; padding: 1.5rem; box-shadow: var(--card-shadow);"
                class="snap-start">
                <h3 style="font-size: 1.1rem; color: var(--title-color); font-weight: 700; margin-bottom: 1.5rem; display: flex; justify-between; align-items: center;"
                    class="justify-between flex">
                    <span>{{ __('messages.open_deals') }}</span>
                    <span
                        style="background: var(--accent-bg); color: var(--accent-text); border-radius: 20px; padding: 0.1rem 0.7rem; font-size: 0.8rem;">{{ $openDeals->count() }}</span>
                </h3>
                <div class="space-y-4 min-h-[400px]" @dragover.prevent @drop.prevent="drop($event, 'open')">
                    @foreach ($openDeals as $deal)
                        <div draggable="true" @dragstart="drag($event, {{ $deal->id }})"
                            ...
                            class="group">
                            <h4
                                style="color: var(--text-color); font-weight: 650; font-size: 1.05rem; margin-bottom: 0.25rem;">
                                {{ $deal->title }}
                            </h4>
                            <p style="color: var(--text-color); opacity: 0.7; font-size: 0.85rem; margin-bottom: 1rem;">🏢
                                {{ $deal->client->name }}
                            </p>
                            <div class="flex justify-between items-center mt-2">
                                <span
                                    style="color: var(--text-color); font-weight: 750; font-size: 1.1rem;">${{ number_format($deal->value, 0) }}</span>
                                <a href="{{ route('deals.show', $deal) }}"
                                    style="color: var(--accent-bg); font-size: 0.8rem; font-weight: 600; text-decoration: none;"
